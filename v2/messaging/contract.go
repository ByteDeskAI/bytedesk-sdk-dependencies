...
//
// v2 carries the v1 contract forward UNCHANGED. The re-platform onto durable
// streams — conversations as stream subjects, receipts as consumer ack floors,
// idempotency as a message id, cursors as sequence numbers — is plan 3, wave 3,
// and SQLite stays the adapter until parity is proven. Changing the wire here
// now would break the running service to no purpose, so the only v2 difference
// is the module it lives in.
package messaging
